...
export const CATALOGS = {
  credenciales: { title: 'Credenciales de equipos', description: 'Cuentas y licencias asociadas. Las contraseñas de los activos no se copian.', columns: [['center_code', 'Código TI'], ['usuario_asignado', 'Usuario'], ['win_usuario', 'Windows'], ['ms_usuario', 'Microsoft'], ['ms_licencia', 'Licencia'], ['ms_vencimiento', 'Vencimiento Microsoft 365'], ['correo_mrt', 'Correo MRT']] },
  ...
  fortigate: { title: 'FortiGate', monitor: true, editable: true, columns: [['software', 'Software'], ['numero_serie', 'Serie'], ['proyecto', 'Proyecto'], ['fecha_expira', 'Vencimiento'], ['ip_address', 'IP'], ['comentario', 'Comentario']] },
  ...
  unidades: { title: 'Catálogo de unidades de inventario', description: 'Nombres de referencia del inventario. Consulta su uso y los equipos asociados en Inventario por unidad.', columns: [['nombre', 'Nombre'], ['activa', 'Activa'], ['orden', 'Orden']] },
...
const CATALOG_CREATE_FORMS = {
  credenciales: { endpoint: '/activos', singular: 'credencial de equipo', note: 'El código TI crea un activo nuevo; después podrás completar su ficha y asignación. Las licencias antivirus se administran en su sección propia.', fields: [['center_code', 'Código TI', 'text', true], ['tipo', 'Tipo', 'select', false, ['Laptop', 'PC', 'Servidor']], ['descripcion', 'Descripción'], ['win_cuenta', 'Cuenta Windows'], ['win_usuario', 'Usuario Windows'], ['ms_cuenta', 'Cuenta Microsoft 365'], ['ms_usuario', 'Usuario Microsoft 365'], ['ms_licencia', 'Licencia Microsoft 365'], ['ms_vencimiento', 'Vencimiento Microsoft 365', 'date'], ['db_cuenta', 'Cuenta Dropbox'], ['db_usuario', 'Usuario Dropbox'], ['db_licencia', 'Licencia Dropbox'], ['correo_mrt', 'Correo MRT', 'email'], ['correo_corporativo', 'Correo corporativo', 'email']] },
  ...
  passwords: { singular: 'contraseña de red', note: 'La contraseña se cifra antes de almacenarse y sólo un administrador puede revelarla.', fields: [['categoria', 'Categoría', 'text', true], ['subcategoria', 'Subcategoría'], ['ip', 'Dirección IP'], ['direccion', 'Dirección / recurso'], ['usuario', 'Usuario'], ['password', 'Contraseña', 'password', true], ['comentario', 'Comentario', 'textarea']] },
  starlink: { singular: 'servicio Starlink', fields: [['correo_cuenta', 'Cuenta de correo', 'email'], ['ubicacion', 'Ubicación'], ['id_starlink', 'ID Starlink', 'text', true], ['version_equipo', 'Versión del equipo'], ['importe_mes', 'Importe mensual', 'number'], ['dia_corte', 'Día de corte'], ['suscripcion', 'Suscripción'], ['cliente', 'Cliente'], ['comentario', 'Comentario', 'textarea']] },
  fortigate: { singular: 'licencia FortiGate', fields: [['software', 'Software'], ['numero_serie', 'Número de serie', 'text', true], ['proyecto', 'Proyecto'], ['fecha_expira', 'Fecha de vencimiento', 'date'], ['ip_address', 'Dirección IP'], ['comentario', 'Comentario', 'textarea']] },
  dominios: { singular: 'dominio', fields: [['dominio', 'Dominio', 'text', true], ['servicios', 'Servicios'], ['fecha_expira', 'Fecha de vencimiento', 'date'], ['status', 'Estado'], ['comentario', 'Comentario', 'textarea']] },
  mantenimientos: { singular: 'mantenimiento', fields: [['center_code', 'Código TI', 'text', true], ['fecha_servicio', 'Fecha de servicio', 'date', true], ['fecha_fin', 'Fecha de término', 'date'], ['tipo_servicio', 'Tipo de servicio', 'text', true], ['descripcion', 'Descripción', 'textarea'], ['tecnico', 'Técnico'], ['proveedor', 'Proveedor'], ['costo', 'Costo', 'number'], ['numero_ticket', 'Número de ticket'], ['estado', 'Estado'], ['garantia_hasta', 'Garantía hasta', 'date'], ['observaciones', 'Observaciones', 'textarea']] },
  unidades: { singular: 'unidad', fields: [['nombre', 'Nombre', 'text', true], ['activa', 'Estado', 'select', false, [['1', 'Activa'], ['0', 'Inactiva']]], ['orden', 'Orden', 'number']] },
};
...
export function AssetSuiteOverviewPage() {
  ...
  return <div className="space-y-7">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-400">MRTI Activos</p><h1 className="mt-1 text-2xl font-bold">Dashboard de activos</h1><p className="mt-1 text-sm text-slate-400">Estado general del inventario, documentación y pendientes que requieren atención.</p></div><div className="flex flex-wrap gap-2"><Link to="/inventario" className="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:bg-slate-800">Abrir inventario</Link><Link to="/alertas" className="rounded-lg border border-amber-500/40 px-4 py-2 text-sm text-amber-300 hover:bg-amber-500/10">Ver todas las alertas</Link>{isAdmin && <button onClick={sync} disabled={syncing} className="rounded-lg bg-sky-500 px-4 py-2 text-sm font-semibold text-[#2a1c05] disabled:opacity-50">{syncing ? 'Sincronizando…' : 'Actualizar desde origen'}</button>}</div></div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Distribution href="/inventario/unidades" title="Activos por unidad" rows={dashboard?.by_unit || []} total={inventory?.activos || 0} />
    </div>
...
export function AssetCatalogPage() {
  ...
  const [rows, setRows] = useState([]); const [q, setQ] = useState(''); const [includeArchived, setIncludeArchived] = useState(false); const [sort, setSort] = useState(''); const [order, setOrder] = useState('asc'); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [revealed, setRevealed] = useState({}); const [creating, setCreating] = useState(false); const [editingRow, setEditingRow] = useState(null); const [monitorRow, setMonitorRow] = useState(null);
  ...
  function load() { if (!config) return Promise.resolve(); setLoading(true); setError(''); setRevealed({}); return apiFetch(`/activos-suite/resources/${resource}?${query}`).then((body) => setRows(body.data)).catch((err) => setError(err.message)).finally(() => setLoading(false)); }
  ...
  async function preview(row) { setError(''); try { await apiPreview(`/activos-suite/documents/${row.id}/download`, row.nombre || row.archivo); } catch (err) { setError(err.message); } }
  ...
  const actions = (isAdmin && config.archivable !== false) || (isAdmin && config.secret) || config.document || (isAdmin && config.editable) || (isAdmin && config.monitor);
  const createConfig = CATALOG_CREATE_FORMS[resource];
  return <div>
    <div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><Link to="/" className="text-sm text-sky-400 hover:underline">← Dashboard de activos</Link><h1 className="mt-2 text-2xl font-bold">{config.title}</h1>{config.description && <p className="mt-1 text-sm text-slate-400">{config.description}</p>}</div>{isAdmin && createConfig && <button type="button" onClick={() => setCreating(true)} className="rounded-lg bg-sky-500 px-4 py-2 text-sm font-semibold text-[#2a1c05] hover:bg-sky-400">+ Nuevo registro</button>}{resource === 'documentos' && <Link to="/inventario" className="rounded-lg border border-sky-500/40 px-4 py-2 text-sm font-medium text-sky-400 hover:bg-sky-500/10">Adjuntar desde un activo</Link>}</div>
    {resource === 'unidades' && <Link to="/inventario/unidades" className="mb-4 inline-block text-sm text-sky-400 hover:underline">Ver uso de las unidades en el inventario →</Link>}
    ...
    {error && <Message error={error} />}
    <div className="overflow-x-auto rounded-xl border border-slate-800"><table className="min-w-full text-sm"><thead className="bg-slate-900 text-left text-slate-400"><tr>{config.columns.map(([key, label]) => <SortableTh key={key} sortKey={key} sort={activeSort} order={order} onSort={changeSort}>{label}</SortableTh>)}{actions && <th className="px-4 py-3 font-medium">Acciones</th>}</tr></thead><tbody>{loading ? <tr><td colSpan={config.columns.length + (actions ? 1 : 0)} className="p-8 text-center text-slate-500">Cargando…</td></tr> : rows.length === 0 ? <tr><td colSpan={config.columns.length + (actions ? 1 : 0)} className="p-8 text-center text-slate-500">Sin resultados</td></tr> : rows.map((row) => <tr key={row.id ?? row.clave} className={`border-t border-slate-800 ${row.archived_at ? 'opacity-55' : 'hover:bg-slate-900/60'}`}>{config.columns.map(([key]) => <td key={key} className="max-w-72 px-4 py-3"><span className="block truncate" title={display(row[key])}>{display(row[key])}</span>{key === config.columns[0][0] && row.record_origin === 'local' && <span className="mt-1 inline-flex rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-400">Creado en MRTI</span>}{key === config.columns[0][0] && row.locally_edited_at && <span className="mt-1 inline-flex rounded-full bg-sky-500/10 px-2 py-0.5 text-[10px] font-medium text-sky-400">Editado a mano — SAP ya no lo sobreescribe</span>}{key === config.columns[0][0] && Object.prototype.hasOwnProperty.call(revealed, row.id) && <span className="mt-2 block whitespace-pre-wrap rounded bg-black/30 p-2 font-mono text-xs text-amber-300">{Object.entries(revealed[row.id]).map(([name, value]) => `${name}: ${value || '—'}`).join('\n')}</span>}</td>)}{actions && <td className="whitespace-nowrap px-4 py-3"><div className="flex gap-2">{config.document && row.local_storage_path && !row.archived_at && <button onClick={() => preview(row)} title="Abrir documento en otra pestaña" className="text-sky-400 hover:underline">Ver</button>}{config.document && row.local_storage_path && <button onClick={() => download(row)} className="text-sky-400 hover:underline">Descargar</button>}{isAdmin && config.secret && <button onClick={() => toggleReveal(row)} className="text-amber-400 hover:underline">{Object.prototype.hasOwnProperty.call(revealed, row.id) ? 'Ocultar claves' : 'Ver claves'}</button>}{isAdmin && config.editable && createConfig && <button onClick={() => setEditingRow(row)} className="text-sky-400 hover:underline">Editar</button>}{isAdmin && config.monitor && <button onClick={() => setMonitorRow(row)} className="text-sky-400 hover:underline">Monitor</button>}{isAdmin && config.archivable !== false && <button onClick={() => toggleArchive(row)} className={row.archived_at ? 'text-emerald-400 hover:underline' : 'text-red-400 hover:underline'}>{row.archived_at ? 'Restaurar' : 'Archivar'}</button>}</div></td>}</tr>)}</tbody></table></div>
    {creating && <CatalogCreateModal resource={resource} config={createConfig} onClose={() => setCreating(false)} onCreated={load} />}
    {editingRow && <CatalogEditModal resource={resource} config={createConfig} row={editingRow} onClose={() => setEditingRow(null)} onSaved={load} />}
    {monitorRow && <MonitorLinkModal assetUid={monitorRow.asset_uid} label={monitorRow.numero_serie || monitorRow.software} onClose={() => setMonitorRow(null)} />}
  </div>;
}

function CatalogCreateModal({ resource, config, onClose, onCreated }) {
  const [values, setValues] = useState(() => Object.fromEntries(config.fields.filter(([, , type]) => type === 'select').map(([key, , , , options]) => [key, Array.isArray(options?.[0]) ? options[0][0] : '']))); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  useEffect(() => {
  ...
  async function submit(event) {
    ...
    try {
      const endpoint = config.endpoint || `/activos-suite/resources/${resource}`;
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/65 p-0 backdrop-blur-sm sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
    <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="catalog-create-title" className="flex max-h-full w-full flex-col overflow-hidden bg-slate-950 shadow-2xl sm:max-h-[85dvh] sm:max-w-4xl sm:rounded-2xl sm:border sm:border-slate-800">
      <header className="flex items-start justify-between gap-4 border-b border-slate-800 px-5 py-4 sm:px-7"><div><p className="text-xs font-semibold uppercase tracking-widest text-sky-400">Nuevo registro</p><h2 id="catalog-create-title" className="mt-1 text-xl font-bold">Crear {config.singular}</h2>{config.note && <p className="mt-1 max-w-2xl text-sm text-slate-500">{config.note}</p>}</div><button type="button" onClick={onClose} disabled={saving} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-slate-100 disabled:opacity-50" aria-label="Cerrar">×</button></header>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-7">{error && <Message error={error} />}<div className="grid grid-cols-1 gap-4 md:grid-cols-2">{config.fields.map((field) => <CatalogCreateField key={field[0]} field={field} value={values[field[0]]} onChange={(key, value) => setValues((current) => ({ ...current, [key]: value }))} />)}</div></div>
      <footer className="flex justify-end gap-2 border-t border-slate-800 bg-slate-900/50 px-5 py-4 sm:px-7"><button type="button" onClick={onClose} disabled={saving} className="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 disabled:opacity-50">Cancelar</button><button type="submit" disabled={saving} className="rounded-lg bg-sky-500 px-5 py-2 text-sm font-semibold text-[#2a1c05] hover:bg-sky-400 disabled:opacity-50">{saving ? 'Guardando…' : 'Crear registro'}</button></footer>
    </form>
  </div>;
}

function CatalogCreateField({ field, value, onChange }) {
  const [key, label, type = 'text', required = false, options = []] = field; const common = 'w-full rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100';
  return <label className={type === 'textarea' ? 'block md:col-span-2' : 'block'}><span className="mb-1 block text-xs text-slate-400">{label}{required ? ' *' : ''}</span>{type === 'select' ? <select className={common} required={required} value={value ?? ''} onChange={(event) => onChange(key, event.target.value)}><option value="">—</option>{options.map((option) => { const [optionValue, optionLabel] = Array.isArray(option) ? option : [option, option]; return <option key={optionValue} value={optionValue}>{optionLabel}</option>; })}</select> : type === 'textarea' ? <textarea className={common} rows={3} required={required} value={value ?? ''} onChange={(event) => onChange(key, event.target.value)} /> : <input className={common} type={type} required={required} step={type === 'number' ? 'any' : undefined} autoComplete={type === 'password' ? 'new-password' : undefined} value={value ?? ''} onChange={(event) => onChange(key, event.target.value)} />}</label>;
}

function CatalogEditModal({ resource, config, row, onClose, onSaved }) {
  const [values, setValues] = useState(() => Object.fromEntries(config.fields.map(([key]) => [key, row[key] ?? '']))); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  useEffect(() => {
    function closeOnEscape(event) { if (event.key === 'Escape' && !saving) onClose(); }
    document.addEventListener('keydown', closeOnEscape); const previousOverflow = document.body.style.overflow; document.body.style.overflow = 'hidden';
    return () => { document.removeEventListener('keydown', closeOnEscape); document.body.style.overflow = previousOverflow; };
  ...
  async function submit(event) {
    event.preventDefault(); setSaving(true); setError('');
    try {
      await apiFetch(`/activos-suite/resources/${resource}/${row.id}`, { method: 'PATCH', body: JSON.stringify(values) });
      await onSaved(); onClose();
    } catch (err) { setError(err.message); } finally { setSaving(false); }
  }
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/65 p-0 backdrop-blur-sm sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
    <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="catalog-edit-title" className="flex max-h-full w-full flex-col overflow-hidden bg-slate-950 shadow-2xl sm:max-h-[85dvh] sm:max-w-4xl sm:rounded-2xl sm:border sm:border-slate-800">
      <header className="flex items-start justify-between gap-4 border-b border-slate-800 px-5 py-4 sm:px-7"><div><p className="text-xs font-semibold uppercase tracking-widest text-sky-400">Edición manual</p><h2 id="catalog-edit-title" className="mt-1 text-xl font-bold">Editar {config.singular}</h2><p className="mt-1 max-w-2xl text-sm text-slate-500">Al guardar, la sincronización automática de SAP deja de sobreescribir este registro.</p></div><button type="button" onClick={onClose} disabled={saving} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-slate-100 disabled:opacity-50" aria-label="Cerrar">×</button></header>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-7">{error && <Message error={error} />}<div className="grid grid-cols-1 gap-4 md:grid-cols-2">{config.fields.map((field) => <CatalogCreateField key={field[0]} field={field} value={values[field[0]]} onChange={(key, value) => setValues((current) => ({ ...current, [key]: value }))} />)}</div></div>
      <footer className="flex justify-end gap-2 border-t border-slate-800 bg-slate-900/50 px-5 py-4 sm:px-7"><button type="button" onClick={onClose} disabled={saving} className="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 disabled:opacity-50">Cancelar</button><button type="submit" disabled={saving} className="rounded-lg bg-sky-500 px-5 py-2 text-sm font-semibold text-[#2a1c05] hover:bg-sky-400 disabled:opacity-50">{saving ? 'Guardando…' : 'Guardar cambios'}</button></footer>
    </form>
  </div>;
}

function MonitorLinkModal({ assetUid, label, onClose }) {
  const [data, setData] = useState(null); const [error, setError] = useState('');
  function load() { obsFetch(assetUid).then(setData).catch((err) => setError(err.message)); }
  useEffect(() => { load(); }, [assetUid]);
  useEffect(() => {
    function closeOnEscape(event) { if (event.key === 'Escape') onClose(); }
    document.addEventListener('keydown', closeOnEscape); const previousOverflow = document.body.style.overflow; document.body.style.overflow = 'hidden';
    return () => { document.removeEventListener('keydown', closeOnEscape); document.body.style.overflow = previousOverflow; };
  }, [onClose]);
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/65 p-0 backdrop-blur-sm sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div role="dialog" aria-modal="true" aria-labelledby="monitor-link-title" className="flex max-h-full w-full flex-col overflow-hidden bg-slate-950 shadow-2xl sm:max-h-[85dvh] sm:max-w-2xl sm:rounded-2xl sm:border sm:border-slate-800">
      <header className="flex items-start justify-between gap-4 border-b border-slate-800 px-5 py-4 sm:px-7"><div><p className="text-xs font-semibold uppercase tracking-widest text-sky-400">MRTI Monitor</p><h2 id="monitor-link-title" className="mt-1 text-xl font-bold">{label || 'Vincular con Monitor'}</h2></div><button type="button" onClick={onClose} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-slate-100" aria-label="Cerrar">×</button></header>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-7"><ObservabilityPanel assetUid={assetUid} data={data} error={error} onChange={load} /></div>
    </div>
  </div>;
}
