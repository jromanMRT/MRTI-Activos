...
export function TechnicalNotesPage() {
  ...
  async function archive(note) {
    ...
    try {
      ...
      setNotice(next ? 'Nota archivada. Puedes restaurarla desde Archivadas.' : 'Nota restaurada.');
  ...
  return <div className="space-y-5">
    <section className="flex flex-wrap items-start justify-between gap-4">
      <div><h1 className="text-2xl font-semibold">Notas técnicas</h1><p className="mt-1 max-w-2xl text-sm text-slate-400">Guarda baterías compatibles, números de parte y referencias de compra. Encuentra cada nota por equipo, serie, repuesto o contenido.</p></div>
      {!editor && <button type="button" className={PRIMARY} onClick={() => { setEditor({ ...EMPTY_NOTE }); setNotice(''); }}>+ Nueva nota</button>}
    </section>
    ...
    {editor && <NoteEditor key={editor.id || 'new'} note={editor} onCancel={() => setEditor(null)} onSaved={() => {
      setEditor(null); setNotice('Nota guardada correctamente.'); setQuery(''); setArchived(false); setPage(1); setSearchParams({}); setRefresh((value) => value + 1);
    }} />}
    <section className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-800 bg-slate-900 p-4">
      <label className="min-w-0 flex-1 text-sm text-slate-300">Buscar notas<input type="search" maxLength={300} className={`${INPUT} mt-1`} value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Ej. batería Dell, TI-00274, serie o número de parte" /></label>
    ...
    {loading ? <p role="status" className="py-8 text-center text-slate-400">Buscando notas…</p> : !error && <>
      <p className="text-sm text-slate-400">{result.total} {result.total === 1 ? 'nota' : 'notas'}{assetUid ? ' del equipo seleccionado' : ''}</p>
      {!result.data.length && <div className="rounded-xl border border-dashed border-slate-700 p-10 text-center"><h2 className="font-semibold">{query ? 'No hay notas que coincidan' : archived ? 'No hay notas archivadas' : 'Tus referencias técnicas, en un solo lugar'}</h2><p className="mt-2 text-sm text-slate-400">{query ? 'Prueba con una parte del modelo, serie o número de parte.' : 'Por ejemplo: el modelo de batería de una laptop, su voltaje, conector y dónde conseguirla.'}</p></div>}
      ...
      {result.total > 30 && <nav aria-label="Páginas de notas" className="flex items-center justify-center gap-4"><button className={BUTTON} disabled={page === 1} onClick={() => setPage((value) => value - 1)}>Anterior</button><span className="text-sm">Página {page} de {Math.ceil(result.total / 30)}</span><button className={BUTTON} disabled={page * 30 >= result.total} onClick={() => setPage((value) => value + 1)}>Siguiente</button></nav>}
    </>}
  </div>;
}
...
function NoteEditor({ note, onCancel, onSaved }) {
  ...
  return <form onSubmit={save} className="space-y-4 rounded-xl border border-sky-500/40 bg-slate-900 p-5" aria-label={note.id ? 'Editar nota técnica' : 'Nueva nota técnica'}>
    <h2 className="text-lg font-semibold">{note.id ? 'Editar nota' : 'Nueva nota'}</h2>
    <fieldset disabled={saving} className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-700 p-4">
        ...
        <p className="mt-1 text-xs text-slate-400">La nota también puede servir como referencia general para varios modelos, sin elegir un equipo.</p>
        {selectedAsset ? <div className="mt-3 flex flex-wrap items-center gap-3 text-sm"><span>{selectedAsset.center_code} · {selectedAsset.modelo} · {selectedAsset.numero_serie}</span><button type="button" className={BUTTON} onClick={() => { setSelectedAsset(null); update('asset_uid', ''); }}>Quitar vínculo</button></div> : <>
          ...
          {!finding && assetQuery.trim() && !assets.length && !assetError && <p className="mt-2 text-xs text-slate-400">Sin coincidencias. Puedes guardar la nota sin vincular un equipo.</p>}
